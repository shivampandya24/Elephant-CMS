        ...
        <div class="col-md-12">
            @can('user_alert_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.user-alerts.create') }}">
                            ...
                            <tbody>
                                @foreach($userAlerts as $key => $userAlert)
                                    <tr data-entry-id="{{ $userAlert->id }}">
                                        ...
                                        <td>
                                            @can('user_alert_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.user-alerts.show', $userAlert->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('user_alert_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.user-alerts.edit', $userAlert->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('user_alert_delete')
                                                <form action="{{ route('frontend.user-alerts.destroy', $userAlert->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('user_alert_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.user-alerts.massDestroy') }}",
